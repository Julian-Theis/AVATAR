...
def generator(x_real, temperature, vocab_size, batch_size, seq_len, gen_emb_dim, mem_slots, head_size, num_heads,
    ...
    gen_x = tf.transpose(gen_x.stack(), perm=[1, 0], name="gen_x_output")  # batch_size x seq_len
...
def discriminator(x_onehot, batch_size, seq_len, vocab_size, dis_emb_dim, num_rep, sn):
    ...
    print(" POOLED OUTPUTS: ", len(pooled_outputs))
    ...
    #logits = tf.reshape(logits, (batch_size, num_rep, -1), name='logits_reshaped')
    #print("NEW LOGITS", logits)

    prob_out = tf.sigmoid(logits, name="prob_out")
    #logits = tf.squeeze(prob_out, -1, name="discriminator-logits")  # batch_size*num_rep

    logits = tf.squeeze(logits, -1, name="discriminator-logits")  # batch_size*num_rep
    return logits


# The discriminator network based on the CNN classifier
def prob_discriminator(x_onehot, batch_size, seq_len, vocab_size, dis_emb_dim, num_rep, sn):
    # get the embedding dimension for each presentation
    emb_dim_single = int(dis_emb_dim / num_rep)
    assert isinstance(emb_dim_single, int) and emb_dim_single > 0
    ...
    print("Prob-Disc Logits", logits)
    logits = tf.reshape(logits, (batch_size, num_rep))

    prob = tf.layers.dense(inputs=logits, units=1, activation=tf.nn.sigmoid)
    print("Prob out", prob)

    prob_out = tf.squeeze(prob, -1, name="prob_disc_out")  # batch_size*num_rep
    return prob_out
